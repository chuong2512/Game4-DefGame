...
public class Master : MonoBehaviour
{
    ...
    public static void WaitAndDo(float time, System.Action onComplete, MonoBehaviour monoBehaviour = null,
        bool isRunWithoutTimeSale = false)
    {
    ...
    IEnumerator WaitAndDoIE(float time, System.Action onComplete = null, MonoBehaviour monoBehaviour = null,
        bool isRunWithoutTimeScale = false)
    {
    ...
    public static void PlayAnimation(Object imageComponent, string pathToTuxturesAnimation, int startIndex,
        int endIndex, float delay, bool isLoop, System.Action onComplete = null, MonoBehaviour mono = null)
    {
        ...
        if (spriteRenderer)
        {
            listTextures = Resources.LoadAll(pathToTuxturesAnimation, typeof(Sprite)).Cast<Sprite>()
                .OrderBy(x => int.Parse(x.name)).ToArray();
            isSprite = true;
        }
        else
        {
            listTextures = Resources.LoadAll(pathToTuxturesAnimation, typeof(Texture2D)).Cast<Texture2D>()
                .OrderBy(x => int.Parse(x.name)).ToArray();
            isSprite = false;
        }

        //Debug.Log(listTextures.Length);
        doPlayAnimation(startIndex, imageComponent, isSprite, listTextures, startIndex, endIndex, delay, isLoop,
            onComplete, mono);
    }

    private static void doPlayAnimation(int index, Object imageComponent, bool isSprite, Object[] listTextures,
        int startIndex, int endIndex, float delay, bool isLoop, System.Action onComplete, MonoBehaviour mono)
    {
        WaitAndDo(delay, () =>
        {
            ...
            doPlayAnimation(index, imageComponent, isSprite, listTextures, startIndex, endIndex, delay, isLoop,
                onComplete, mono);
        }, mono);
    }
